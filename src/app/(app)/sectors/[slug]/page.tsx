...
export default async function SectorDetailPage({
  ...
  return (
    <div className="flex flex-col flex-1 px-4 py-5 gap-5">
      ...
      <Link
        href="/sectors"
        className="flex items-center gap-1 text-xs font-medium text-stone-500 hover:text-stone-700 transition-colors w-fit"
      >
      ...
      <div className={`rounded-xl border ${colors.border} ${colors.bg} p-4`}>
        ...
        <p className="text-xs text-stone-600 mt-1 leading-relaxed">{sector.description}</p>
      ...
      <Link
        ...
        className="w-full py-3 rounded-xl bg-stone-900 text-white text-sm font-semibold text-center hover:bg-stone-800 transition-colors flex items-center justify-center gap-2"
      >
      ...
      {/* Ticker list */}
      <div>
        <p className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-2.5">
        ...
        <div className="flex flex-col gap-2">
            ...
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-stone-900">{q.symbol}</span>
                  <span className="text-[10px] text-stone-400 truncate max-w-[140px]">{q.name}</span>
                </div>
              ...
              <div className="text-right shrink-0 ml-3">
                ...
                <p
                  className={`text-xs font-semibold ${
                    q.changePct >= 0 ? "text-emerald-600" : "text-red-600"
                  }`}
                >
                  {q.changePct >= 0 ? "+" : ""}
          ...
          {sortedQuotes.length === 0 && (
            <div className="text-center py-8">
              <p className="text-sm text-stone-500">Could not fetch quotes for this sector.</p>
            </div>
          )}
        </div>
      </div>
