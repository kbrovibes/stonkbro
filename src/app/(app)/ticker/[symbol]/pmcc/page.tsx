  ...
  return (
    <div className="rounded-xl border border-stone-200 bg-white p-4">
      ...
      <div className="flex items-center gap-4 mb-3">
        <div>
          <p className="text-lg font-bold text-stone-900">${leg.strike.toFixed(2)}</p>
      ...
      <div className="grid grid-cols-4 gap-2">
        <div className="text-center p-2 rounded-lg bg-stone-50">
          <p className="text-xs font-semibold text-stone-700">{delta.toFixed(2)}</p>
          <p className="text-[10px] text-stone-400">Delta</p>
        </div>
        <div className="text-center p-2 rounded-lg bg-stone-50">
          <p className="text-xs font-semibold text-stone-700">{(leg.impliedVolatility * 100).toFixed(0)}%</p>
          <p className="text-[10px] text-stone-400">IV</p>
        </div>
        <div className="text-center p-2 rounded-lg bg-stone-50">
          <p className="text-xs font-semibold text-stone-700">{leg.volume.toLocaleString()}</p>
          <p className="text-[10px] text-stone-400">Vol</p>
        </div>
        <div className="text-center p-2 rounded-lg bg-stone-50">
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-2">
        <h3 className="text-sm font-bold text-stone-900">Setup #{index + 1}</h3>
        <GradeBadge grade={setup.grade} />
        {setup.signals.length > 0 && (
          <span className="text-[10px] text-stone-400 ml-auto">{setup.signals.length} signal{setup.signals.length !== 1 ? "s" : ""}</span>
        )}
      </div>
      ...
      {setup.signals.length > 0 && (
        <div className="rounded-xl border border-stone-200 bg-white p-4">
          <h3 className="text-sm font-bold text-stone-900 mb-2">Signals</h3>
          <div className="flex flex-wrap gap-1.5">
            {setup.signals.map((sig, i) => (
              <span key={i} className="text-[10px] font-medium bg-stone-50 text-stone-600 px-2 py-1 rounded-full">
                {sig}
              </span>
            ))}
          </div>
      ...
      <div className="rounded-xl border border-stone-200 bg-white p-4">
        ...
        <div className="flex items-end gap-1 h-20">
          <div className="flex-1 flex flex-col items-center gap-1">
            <div className="w-full bg-red-100 rounded-t" style={{ height: "60px" }} />
            <span className="text-[10px] text-red-600 font-semibold">Max Risk</span>
            <span className="text-[10px] text-stone-500">{formatCurrency(setup.maxRisk)}</span>
          </div>
          <div className="flex-1 flex flex-col items-center gap-1">
            <div className="w-full bg-emerald-100 rounded-t" style={{ height: `${Math.min((setup.monthlyPremium / setup.maxRisk) * 400, 60)}px` }} />
            <span className="text-[10px] text-emerald-600 font-semibold">Monthly</span>
            <span className="text-[10px] text-stone-500">{formatCurrency(setup.monthlyPremium)}</span>
          </div>
          <div className="flex-1 flex flex-col items-center gap-1">
            ...
            <span className="text-[10px] text-stone-500">{formatCurrency(setup.monthlyPremium * 12)}</span>
          </div>
        </div>
      </div>
    </div>
...
export default async function PMCCPage({ params }: { params: Params }) {
    ...
    return (
      <div className="flex flex-col flex-1 px-4 py-5 gap-5">
        ...
        <div className="flex flex-col items-center justify-center flex-1 text-center">
          ...
          <h2 className="text-lg font-bold text-stone-900">Quote not found</h2>
          <p className="text-sm text-stone-500 mt-1">Could not fetch market data for {upper}.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col flex-1 px-4 py-5 gap-5">
        ...
        <div className="flex flex-col items-center justify-center flex-1 text-center">
          ...
          <h2 className="text-lg font-bold text-stone-900">No options data available</h2>
          <p className="text-sm text-stone-500 mt-1">Options chain not available for {upper}. This ticker may not have listed options.</p>
        </div>
      </div>
    );
  }
  ...
  if (setups.length === 0) {
    return (
      <div className="flex flex-col flex-1 px-4 py-5 gap-5">
        <BackLink symbol={upper} />
        <div>
          <h2 className="text-lg font-extrabold text-stone-900">PMCC Setup · {upper}</h2>
        ...
        <div className="flex flex-col items-center justify-center flex-1 text-center">
          ...
          <h2 className="text-lg font-bold text-stone-900">No PMCC setups found</h2>
  ...
  return (
    <div className="flex flex-col flex-1 px-4 py-5 gap-4">
      ...
      {/* Setup count */}
      <div className="flex items-center gap-2">
        <span className="text-xs text-stone-500">{setups.length} setup{setups.length !== 1 ? "s" : ""} found</span>
        <div className="flex gap-1">
          {(["A", "B", "C"] as const).map((g) => {
            const count = setups.filter((s) => s.grade === g).length;
            return count > 0 ? (
              <span key={g} className="text-[10px] text-stone-400">
                {count}{g}
              </span>
            ) : null;
          })}
      ...
      {setups.map((setup, i) => (
        <div key={i}>
          {i > 0 && <hr className="border-stone-200 mb-4" />}
