...
function TickerChipInput({
  ...
  return (
    <div
      className="flex flex-wrap gap-1.5 min-h-[48px] px-3 py-2 rounded-xl border border-stone-200 bg-white cursor-text"
      onClick={() => inputRef.current?.focus()}
    >
      {value.map((t) => (
        <span
          key={t}
          className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-stone-900 text-white text-xs font-bold"
        >
          {t}
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); onChange(value.filter((v) => v !== t)); }}
            className="text-stone-400 hover:text-white ml-0.5 leading-none"
          >
            ×
          </button>
        </span>
      ...
      <input
        ...
        className="flex-1 min-w-[80px] text-sm font-medium text-stone-900 placeholder:text-stone-400 outline-none bg-transparent"
      />
    </div>
  );
}
...
function TickerSuggestionCard({ symbol }: { symbol: string }) {
  return (
    <Link
      ...
      className="flex items-center justify-between p-4 rounded-xl border border-stone-200 bg-white hover:border-stone-300 hover:shadow-sm transition-all"
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-bold text-stone-900">{symbol}</p>
          <p className="text-xs text-stone-400">Tap to see CSP · CC · PMCC</p>
        </div>
      </div>
    ...
    <div className="flex flex-col flex-1 px-4 py-5 gap-5">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <button
          ...
          className="w-full py-3 rounded-xl bg-stone-900 text-white text-sm font-semibold disabled:opacity-40 disabled:cursor-not-allowed hover:bg-stone-800 transition-colors"
        >
          {tickers.length === 0
            ? "Add tickers above"
            : tickers.length === 1
