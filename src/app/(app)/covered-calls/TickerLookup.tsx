    ...
    <div className="flex flex-col gap-3">
      <form onSubmit={handleSubmit} className="flex items-end gap-2">
        <div className="flex flex-col gap-1">
          <label className="text-[10px] text-stone-400 uppercase tracking-wider font-semibold">Symbol</label>
          <input
            type="text"
            value={symbol}
            onChange={(e) => setSymbol(e.target.value.toUpperCase())}
            placeholder="AAPL"
            className="w-24 px-3 py-2 text-sm font-semibold text-stone-900 rounded-lg border border-stone-200 bg-white focus:outline-none focus:ring-2 focus:ring-stone-300 placeholder:text-stone-300"
          />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-[10px] text-stone-400 uppercase tracking-wider font-semibold">Cost Basis</label>
          <input
            type="number"
            step="0.01"
            value={costBasis}
            ...
            placeholder="0.00"
            className="w-28 px-3 py-2 text-sm text-stone-900 rounded-lg border border-stone-200 bg-white focus:outline-none focus:ring-2 focus:ring-stone-300 placeholder:text-stone-300"
          />
        </div>
        <button
          type="submit"
          disabled={isPending || !symbol.trim()}
          className="px-4 py-2 text-xs font-semibold text-white bg-stone-900 hover:bg-stone-800 disabled:bg-stone-300 rounded-lg transition-colors"
        >
      ...
      {result && !result.error && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-bold text-stone-900">{result.symbol}</span>
            <span className="text-sm font-semibold text-stone-700">{formatDollar(result.currentPrice)}</span>
          </div>
