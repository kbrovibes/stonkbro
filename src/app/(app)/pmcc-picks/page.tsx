...
function RankBadge({ rank }: { rank: number }) {
  const bg = rank <= 3 ? "bg-stone-900 text-white" : "bg-stone-200 text-stone-600";
  return (
...
export default function PMCCPicksPage() {
  ...
  return (
    <div className="flex flex-col flex-1 px-4 py-5 gap-5">
      {/* Header */}
      <div>
        <h2 className="text-lg font-extrabold text-stone-900">PMCC Picks</h2>
      ...
      {scanning && (
        <div className="flex items-center gap-3 py-3">
          ...
          <span className="text-xs text-stone-500">{scanProgress || "Scanning options chains..."}</span>
        </div>
      )}
      ...
      {scanComplete && (
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4 text-xs text-stone-500">
            ...
            {gradeACount > 0 && <span className="text-emerald-600 font-semibold">{gradeACount} Grade A</span>}
          </div>
            ...
            {([["grade", "Grade"], ["return", "Return"], ["capital", "Capital"]] as const).map(([mode, label]) => (
              <button
                ...
                className={`px-2 py-1 rounded-md text-[10px] font-semibold transition-colors ${
                  sortMode === mode ? "bg-stone-900 text-white" : "bg-stone-100 text-stone-500 hover:bg-stone-200"
                }`}
              >
                {label}
              </button>
      ...
      {sorted.length > 0 && (
        <div className="flex flex-col gap-3">
          {sorted.map((setup, i) => {
            const rank = i + 1;
            const gradeColor = setup.grade === "A" ? "border-emerald-200" : setup.grade === "B" ? "border-amber-200" : "border-stone-200";
            ...
            return (
              <div key={`${setup.symbol}-${setup.leaps.strike}-${setup.shortCall.strike}-${i}`} className={`rounded-xl border ${gradeColor} bg-white overflow-hidden`}>
                {/* Card header */}
                <div className="flex items-center justify-between px-4 pt-4 pb-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-xs text-stone-400">{fmtDec(setup.stockPrice)}</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 px-4 pb-3">
                  <div className="p-2.5 rounded-lg bg-sky-50 border border-sky-100">
                    <p className="text-[10px] font-semibold text-sky-600 uppercase tracking-wide">Buy LEAPS</p>
                    <p className="text-sm font-bold text-stone-900 mt-1">${setup.leaps.strike} Call</p>
                    <p className="text-[10px] text-stone-500">{setup.leaps.expiry} &middot; {setup.leaps.dte}d</p>
                    <p className="text-xs font-semibold text-stone-700 mt-1">{fmtDec(setup.leaps.mid)} &middot; &delta; {setup.leapsDelta.toFixed(2)}</p>
                  </div>
                  <div className="p-2.5 rounded-lg bg-amber-50 border border-amber-100">
                    <p className="text-[10px] font-semibold text-amber-600 uppercase tracking-wide">Sell Call</p>
                    <p className="text-sm font-bold text-stone-900 mt-1">${setup.shortCall.strike} Call</p>
                    <p className="text-[10px] text-stone-500">{setup.shortCall.expiry} &middot; {setup.shortCall.dte}d</p>
                    <p className="text-xs font-semibold text-emerald-700 mt-1">{fmtDec(setup.shortCall.mid)} &middot; &delta; {setup.shortDelta.toFixed(2)}</p>
                  </div>
                </div>
                ...
                <div className="border-t border-stone-100 px-4 py-3">
                  <div className="grid grid-cols-4 gap-2">
                    <div className="text-center">
                      <p className="text-xs font-bold text-stone-900">{fmt(setup.capitalRequired)}</p>
                      <p className="text-[10px] text-stone-400">Capital</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xs font-bold text-emerald-700">{fmt(setup.monthlyPremium)}</p>
                      <p className="text-[10px] text-stone-400">Monthly</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xs font-bold text-stone-900">{setup.monthlyReturnPct.toFixed(1)}%</p>
                      <p className="text-[10px] text-stone-400">Mo Return</p>
                    </div>
                    <div className="text-center">
                ...
                <div className="border-t border-stone-100 px-4 py-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-stone-500">
                      Saves <span className="font-semibold text-stone-700">{setup.capitalVs100Shares.toFixed(0)}%</span> vs 100 shares
                    </span>
                    <span className="text-stone-500">
                      12mo: <span className="font-semibold text-emerald-700">{fmt(setup.incomeProjection12mo)}</span> on {fmt(setup.capitalRequired)}
                    </span>
                  </div>
                </div>
                ...
                {setup.signals.length > 0 && (
                  <div className="border-t border-stone-100 px-4 py-2.5 flex flex-wrap gap-1.5">
      ...
      {!scanning && !scanComplete && (
        <div className="flex flex-col items-center justify-center flex-1 text-center py-12">
          ...
          <h3 className="text-sm font-bold text-stone-900">Ready to find PMCC setups</h3>
      ...
      {scanComplete && setups.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          <h3 className="text-sm font-bold text-stone-900">No PMCC setups found</h3>
          <p className="text-xs text-stone-500 mt-1">Try scanning a different sector or all tickers.</p>
        </div>
      )}
    </div>
  );
