/**
 * Server-safe nav data. Next 16 refuses to CALL functions imported from a
 * "use client" module inside a Server Component, which broke /more —
 * keep the data + filter here (no directive) and the interactive tile in
 * MoreNav.tsx.
 */
...
export const MORE_GROUPS: MoreGroup[] = [
  ...
  {
    // Unlike the group below, Portfolio itself is reachable by anyone
    // signed in — the invite-only gate lives on the page now (a
    // request-access prompt), not in the nav, so people can actually find
    // it to ask.
    label: "Portfolio",
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
