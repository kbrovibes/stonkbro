 ...
 * Uses per-account position calls (the /holdings bulk endpoint is deprecated).
 */
...
const txApi = new TransactionsAndReportingApi(config);
  ...
  const all = await Promise.all(
    accounts.map(async (acct) => {
      ...
      return positions.map((p: any): Position => {
        ...
        const desc: string = p.symbol?.symbol?.description ?? "";
        ...
        const isOption = !!p.symbol?.option_symbol;
        return {
          symbol: sym,
          description: desc,
          units,
...
export async function getTransactions(days = 90) {
  ...
  const res = await txApi.getActivities({ userId: UID, userSecret: USEC, startDate: start, endDate: end });
