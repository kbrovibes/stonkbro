...
- **Single playlist, not grouped by day**: the Daily Briefing player showed a new date-header section per day; it's one continuous newest-first list now, each row carrying its own date/session/time inline
- **Tap the cover to play/pause**: the album art is a real play/pause button with a hover/tap overlay, not just decoration
- **Prev/next episode buttons**: previously only 15-second skip existed, nothing moved between episodes without going back to the playlist. Added dedicated prev/next controls that walk the full chronological queue
- **Auto-play on open, with a 3-state control**: the newest episode now starts automatically when the page loads (best-effort — browsers routinely block autoplay with no prior gesture, handled silently rather than surfaced as an error). A new pill button cycles auto-advance-on-end through **same-day** (default, continues within the day and stops), **across-days** (keeps going into earlier days), and **off** (stops after the current clip) — persisted in localStorage

## v0.42.0 — Portfolio opens up, ticker chart tabs, option-chain fixes

- **Portfolio is now request-based, not hardcoded**: anyone signed in sees the Portfolio nav entry and, if not yet approved, gets a "Request access" prompt right on the page instead of a dead end. The owner approves or denies from the alert banner (new Approve/Deny buttons — the first alert type with real actions) or from a new admin-only section in Settings. Approval registers the requester as their own SnapTrade end-user and sends them through SnapTrade's own hosted connect portal to link a brokerage — this app never sees their brokerage login. Their SnapTrade identity is stored AES-256-GCM-encrypted (`src/lib/snaptrade/secrets.ts`) in a new table with no client-readable RLS policy at all, not even for the owning user — decryption happens only server-side, only at the moment of an actual SnapTrade call. `src/lib/snaptrade/client.ts` now takes an optional per-call SnapTrade identity everywhere, defaulting to the app's existing owner identity when omitted, so the owner's own flow (and its cron-cached chain scan) is unchanged; other approved users always fetch live. Time Machine, Taxes, and Daily Briefing stay on the original owner-only allowlist for now
- **Ticker page gets a real price chart**: the default (refresh) theme's ticker screen showed only a fixed sparkline with IV/IVR/RSI/VOL badges and no way to see price history — confusing enough that it read as "some IV chart nobody knows how to use." A new Price/Stats switch defaults to a proper time-window chart (1D/1W/1M/3M/1Y/YTD, hover for date and %, matching what the classic theme already had); the old badge card moves to Stats
- **Option chains: roll ordering fixed**: a same-day roll (buy-to-close the old contract, sell-to-open the new one) was rendering as SELL, SELL, BUY instead of SELL, BUY, SELL — a display-only re-sort forced every SELL before every BUY on a date tie, which is right for disambiguating a single contract's own same-day open+close but wrong for merging a roll's two different contracts. Removed; the already-correct roll order needed no re-sort
- **Option chains: a fully-rolled position no longer fragments into two**: a "settle stale history" rule split a lineage's oldest contracts off once it spanned more than 42 days, so a call rolled continuously (never gone flat) since before that window showed as a fake "closed" chain plus a fake "new" open one. Now only settled (actually closed/expired/assigned) lineages get trimmed — a still-open chain keeps its full roll history back to origin
- **Option chains: partial rolls now link instead of looking unrelated**: rolling some but not all contracts of a short position (buy back 2 of 5, resell those 2 at a new strike, leave 3 open) showed the new strike as a completely disconnected position. Chains now carry `splitFrom`/`splitInto` links plus a `groupId`, `groupStatus`, and `groupNetPnl` rolled up across every linked piece — "is the overall trade still open, was it profitable" is answered directly off either chain. The Portfolio UI marks both halves with a `SPLIT` badge and the group's overall status/P&L
- **Portfolio page can no longer render silently blank**: a failed initial chain load with no offline signal returned `null` — no error, no retry, nothing. Now shows an explicit "Couldn't load your portfolio / Try again" card
- **Paper desk home page shows cash and overall return**: the hero previously showed only the combined gain/loss dollar figure; it now also shows the overall gain/loss percentage next to it, plus Portfolio value and Cash stat tiles across all ten bots

## v0.41.0 — Paper desk: personas, memory, and a backfill harness

- **The ten bots got names and faces**: Atlas (index-dca), Compass (sector-rotator), Northstar (megacap-momentum), Booster (margin-bull), Salvage (dip-buyer), Breakwater (put-seller), Wheelhouse (wheel), Longview (pmcc-operator), Canopy (spy-condor), Vector (growth-shadow) — profile ids are unchanged, only the display names. New `src/lib/paper/identity.ts` holds each bot's role label, creed, hue and face config; `src/components/paper/BotAvatar.tsx` draws the portrait as inline SVG (no image assets, no CDN)
- **Soul: every bot now remembers**: new `paper_memory` table (migration `20260909010000_paper_memory.sql`, applied) and `src/lib/paper/memory.ts` derive six kinds of memory from each close — creed, conviction, lesson, scar, streak, milestone. Re-observing a memory bumps its hit count instead of writing a duplicate, so a repeated lesson gets heavier rather than longer. The runner writes memories after the close session. **Memory never feeds back into order generation** — it is character, not signal
- **Bots make room instead of giving up**: new `src/lib/paper/funding.ts` — when an order is rejected for buying power, the bot closes its weakest holding (worst unrealised return) to fund it, and both trades record reasons saying so. Two hard guards: it never sells shares or a long call that is covering a short call, and it never lifts one leg off a multi-leg group. Wired into `src/lib/paper/engine.ts`, which now also executes closing orders before opening ones. Validated by `scripts/validate-paper-funding.ts` (26 checks)
- **The broker will not strand a short leg**: `executeOrder` rejects any order that would leave a short position uncovered — selling the shares behind a covered call, closing the LEAPS under a PMCC while the short call is open, or lifting a condor wing while its short leg is still there. Strategies close the short leg first, but that was a convention rather than a guarantee: a rejected close still let the cover be sold. Two unbounded-loss paths in Longview came out of a static audit, neither reachable by a uniformly rising or falling month, and both are now covered by regression tests
- **The broker will not write an uncovered short call**: `marginMode: "covered"` declares a margin requirement of zero, so the buying-power test had nothing to reject and a naked call opened for free. Longview could stack a ratio spread by one contract per session after a single failed buy-to-close, with everything past the first contract naked
- **Booster no longer re-levers in the tick it de-risks**: `momentum.ts` sold its weakest name to cut borrowing and then bought a replacement at 32% of equity in the same pass, using buying power that still included the whole margin line, which made its $85K floor decorative. A tick that de-risks now does nothing else
- **Breakwater carries a book-wide obligation ceiling**: contract sizing rounds down to whole contracts, so one contract on an expensive name wrote more notional than the $10K-per-contract heuristic asked for. Total short-put obligation is now capped at 2.5× equity
- **Equity floors on the margin bots**: Vector previously added 25% of equity on margin on every 3% basket drop with no limit — it now stops adding within 5% of an $85,000 equity floor, never borrows more than 50% of equity, and sells down to zero borrowing under the floor. Breakwater stops opening new puts under the same floor
- **Backfill harness**: `src/lib/paper/backfill.ts` replays past sessions entirely in memory against recorded daily bars, never touching Supabase; `src/lib/paper/synthetic.ts` models option chains with Black–Scholes off realised volatility, because no provider serves a historical chain. Scripts: `paper-fetch-bars.ts` (caches bars), `paper-backfill.ts` (runs a window), `paper-report.ts` (writes the report), `validate-paper-invariants.ts` and `validate-paper-funding.ts`; the shared safety checks live in `src/lib/paper/invariants.ts`
- **Paper screens redesigned**: `/paper` is now a ranked two-column board with avatars, rank numbers, persona name and role. `/paper/[profileId]` gains a hero with the avatar and creed, a four-cell stat strip, the equity chart beside a Rulebook card, and four counted tabs — Soul, Positions, Trades, Journal. Positions render as a proper table (position, type, qty, avg, mark, market value, unrealized). Both API routes now return identity and memory
- **Simulated report**: `reports/paper-august-2026.html` and `.md` — a backfilled August 2026 across all ten bots

## v0.40.0 — LEAPS Lab + PMCC Income, Paper Trading, home card grid

- **Home is a card grid again**: the refresh Pulse home keeps its index hero and breadth bar but swaps the short movers list for 4-column ticker cards with sparklines — Explosive movers, each of your watchlists (avg return, Manage link), and Today's winners/losers when you have no watchlists. Held and at-risk tickers carry HELD / RISK badges. Feature tiles (Bloodbath, Portfolio, Hindsight, Paper, Briefing) sit under the index tiles
- **Podcast is back**: the Daily Briefing card returns to the home screen for portfolio-access users and Daily Briefing is listed in the More menu (Portfolio group)
- **LEAPS Lab** (Options → LEAPS, `/plays?s=leaps`): a twice-daily cron scans ~110 liquid names and scores the best LEAPS to buy (liquidity 40%, moderate IV 25%, momentum 25%, 52-week proximity 10%) with a "why it ranks" line and recommended / ITM / OTM strikes. Pick or pin a ticker and the Lab prices strikes off the live chain and renders a return grid — quarters to expiry across, −30% to +100% stock move down — with the contract value in every cell, Black–Scholes with IV held constant. Add your own strikes, compare two strikes (with a delta grid), keep notes; pins, strikes and the captured recommendation persist per user (`leaps_scans`, `leaps_pins`)
- **PMCC Income** (Options → PMCC, `/plays?s=pmcc`): enter a capital budget (default $20K) and the day's PMCC setups rank by monthly income — spreads affordable, monthly and annual income, annualized ROC, PMCC score — with a full per-spread breakdown: the LEAPS you buy (midpoint, IV, OI, DTE) versus the call you sell (credit, monthly per spread, OI), net debit, breakeven, spread width, max profit at short expiry, and a "how the income estimate works" note. Re-ranks instantly as the budget changes
- **Paper Trading** (`/paper`, new bottom-nav tab): ten bots with written plans — Index DCA, Sector Rotation, Mega-cap Momentum, Margin Bull, Dip Buyer, Naked Put Seller, The Wheel, PMCC Operator, SPY Weekly Iron Condor, Growth Shadow (iofund-style proxy basket) — each start with $100K cash and a $200K margin line. Crons at the open, midday and close mark every account on live quotes and chains, run each bot's rules, fill at the quote (options at mid ± 25% of spread, $0.65/contract), settle expiries, charge 8% margin interest, and persist every trade. The close run writes a daily snapshot plus highlights, learnings and an AI narrative per bot and a desk-wide note. Leaderboard with equity sparklines and TODAY / TOTAL toggle; per-bot page with plan, stat tiles, equity chart, day picker, positions, trades and notes. Admins can run a session on demand
- **More menu**: LEAPS Lab, PMCC Income, a new Paper group, Daily Briefing
- 7 new `paper_*` tables and 2 `leaps_*` tables; 5 new cron entries in `vercel.json`
