...
- **The broker will not strand a short call**: `executeOrder` now rejects any order that would leave a short call uncovered — selling the shares behind a covered call, or closing the LEAPS under a PMCC while the short leg is still open. Strategies already close the short leg first, but that was a convention rather than a guarantee; if the short-leg close were ever rejected, the cover would still have been sold
