...
Ten paper-trading bots — Atlas (index DCA), Compass (sector rotation), Northstar (mega-cap momentum), Booster (leveraged momentum), Salvage (dip buying), Breakwater (naked puts), Wheelhouse (the wheel), Longview (LEAPS and short calls), Canopy (weekly iron condor) and Vector (growth basket) — each with a written plan, a drawn face and a creed, start with $100,000 cash and a $200,000 margin line. Three crons a day (open, midday, close) mark every account to market on live quotes and chains, let each bot act on its rules, fill at the quote, settle expiries, charge margin interest, and persist every trade. A bot refused for buying power closes its weakest holding to fund the trade — never one that covers a short call, never one leg of a multi-leg group. The close run writes a daily snapshot plus highlights, learnings and a short narrative per bot and a desk-wide note. `/paper` is a ranked board of avatars; tap a bot for its hero, rulebook, equity chart and five tabs — Soul, Positions, Trades, Portfolio, Journal. Soul/Positions/Trades/Journal are scoped to one selected day; Portfolio is the bot's full-history view instead — every trade it's ever made, every closed position with its realized P&L and return %, independent of the day picker. Paper tab in the bottom nav.

### Bot Memory (Soul)
Every paper bot keeps a persistent memory alongside the daily notes: its founding **creed**, **convictions** the record keeps confirming, **lessons** from how trades went, **scars** worth not repeating, **streaks**, and **milestones** crossed. Memories are derived after each close and deduplicated — re-observing one bumps its hit count instead of adding a row, so a repeated lesson gets heavier rather than the list getting longer. Memory never feeds back into order generation; it is character, not signal.

### Paper Backfill Harness
A local harness replays past paper sessions entirely in memory against recorded daily bars, without touching Supabase. Because no provider serves a historical option chain, `src/lib/paper/synthetic.ts` models one with Black–Scholes off realised volatility. Run it from `scripts/`: `paper-fetch-bars.ts` caches the bars, `paper-backfill.ts` replays a window across all ten bots, `paper-report.ts` writes the HTML and Markdown report, and `validate-paper-invariants.ts` / `validate-paper-funding.ts` assert the engine's safety rules. Backfilled option results are modelled, not recorded quotes.
...
### Daily Audio Briefing (3x-daily podcast)
A personal ~2–3 minute audio briefing generated three times every trading day — pre-market (6:00am PT), an hour after the open (7:30am PT), and after the close with after-hours trends (2:00pm PT), curated to your portfolio: market pulse, your biggest movers and why, news/earnings that matter today, and concrete trade suggestions (close / roll / open / watch, each with a reason). Free Microsoft Edge neural TTS (no API key), synthesized in small sentence-aligned chunks and retried per-chunk for reliability, renders ~1 MB mono MP3s; a home-screen card opens a Spotify-style player — tap the cover to play/pause, a single chronological playlist (no per-day grouping), prev/next episode buttons, 3-state auto-play (off / same-day / across days, the latest episode starts automatically on open), generated cover art, speed control, transcript, 7-day history, offline download, and force-regenerate. No account-level dollar amounts spoken, by design.
...
src/
  ...
  lib/
    ...
    paper/                Paper-trading desk (profiles, identity, broker, engine,
                          strategies, memory, funding, backfill harness)
    research/             Claude AI analysis engine
    notifications/        Email service (Resend)
scripts/                  Local harness: paper backfill, report, validators
```
...
| `/api/paper` | GET | Paper-trading board: ten bots with identity (name, role, creed, face), accounts, latest snapshots, equity series, desk note |
| `/api/paper/[profileId]?date=` | GET | One bot's identity, memory, account, positions, trades and notes for a day, plus its equity history |
| `/api/paper/[profileId]/portfolio` | GET | One bot's full-history view: every trade ever made and every position ever closed (with realized P&L), independent of the day picker |
